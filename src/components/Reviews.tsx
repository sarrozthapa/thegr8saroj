...
function ReviewColumn({
  ...
  return(
    <div ref={columnRef} className={cn("animate-marquee space-y-8",className)}
    style={{
...
function Review({imgSrc,className,...props}:ReviewProps){
  ...
  const POSSIBLE_ANIMATION_DELAYS=['0s','0.1s',
    '0.2s',
    '0.3s',
    '0.4s',
    '0.5s',
...
function ReviewGrid(){
 ...
 return <div ref={containerRef} className='relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3'>
        ...
        msPerPixel={10}
        />
        <ReviewColumn reviews={[...column2,...column3[1]]} reviewClassName={(reviewIndex)=>cn({"md:hidden":reviewIndex>=column1.length+column3[0].length,
          'lg:hidden':reviewIndex>=column1.length,
        })}
        msPerPixel={15}
        />
        <ReviewColumn reviews={column3.flat()} className='hidden md:block' reviewClassName={(reviewIndex)=>reviewIndex>=column2.length?'lg:hidden':''}
        msPerPixel={10}
        />
        </>:<>
        </>}
